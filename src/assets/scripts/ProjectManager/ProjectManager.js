...
module.exports = () => {
    ...
    const reset = () => {
        try {
            ...
            Object.keys(Module.animationids).forEach((key) => {
                try { cancelAnimationFrame(key); } catch (error) { }
            })
            Module.animationids = {};

            for (let [key, media] of Module.videoids) {
                try { if (media.destroy !== undefined) media.destroy(); } catch (error) { }
            }
            Module.videoids.clear();

            try { Module.clearAllSockets(); } catch (error) { }

            try { Scenegraph.reset(); } catch (error) { }
    ...
    const loadURL = (url, password) => {
        ...
        if (Module['canvas'])
        {
           ...
           if (console_el) console_el.style.display = (console_on === null || console_on[1] != "on" ? "none" : "block");
        }

        URLLoader.visible = true;
        URLLoader.getPackage(url, (event) => {
            if (event) {
                // Load Project
    ...
    const loadFromFolder = function (path) {
        ...
        loadScene({ data: JSON.parse(surface.readFile(path + "project.json")) }, true);
    }
    ...
    Object.defineProperties(manager, {
        path: { get: () => { return Scenegraph.path; }, set: (v) => { Scenegraph.path = v; } },
        project: { get: () => { return Scenegraph.project; }, set: (v) => { Scenegraph.project = v; } },
        objects: { get: () => { return Scenegraph.objects; }, set: (v) => { Scenegraph.objects = v; } },
        isPlaying: { get: () => { return isPlaying; }, set: (v) => { isPlaying = v; } },
        redraw: { get: () => { return Scenegraph.redraw; }, set: (v) => { Scenegraph.redraw = v; } },
        isDirty: { get: () => { return isDirty; }, set: (v) => { isDirty = v; } },
        payload: { get: () => { return payload; }, set: (v) => { payload = v; } },
        objectControllers: { get: () => { return Scenegraph.objectControllers; }, set: (v) => { Scenegraph.objectControllers = v; } },
        meshControllers: { get: () => { return Scenegraph.meshControllers; }, set: (v) => { Scenegraph.meshControllers = v; } },
        worldController: { get: () => { return Scenegraph.worldController; }, set: (v) => { Scenegraph.worldController = v; } },
        projectRunning: { get: () => { return projectRunning; }, set: (v) => { projectRunning = v; } },
        objPaths: { get: () => { return Scenegraph.objPaths; }, set: (v) => { Scenegraph.objPaths = v; } },
        
        computedStyles: { get: () => { return computedStyles; }, set: (v) => {  } },

        treeGenerated: { get: () => { return Scenegraph.treeGenerated; }, set: (v) => { Scenegraph.treeGenerated = v; } },
