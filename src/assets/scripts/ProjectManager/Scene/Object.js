...
module.exports = (payload) => {
    ...
    const playAnimation= (animation)=> {
        ...
        if (!object || animation_id == undefined || isNaN(animation_id) || animation_id >= animation_list.length ) return;
        ...
        if (animation.end == undefined && details.endTime != undefined) animation.end = details.endTime;
        if (animation.start == undefined && details.startTime != undefined) animation.start = details.startTime;
       
        if (animation.raw){
            if (animation.end != undefined && animation.end >= 0) newEnd = animation.end;
            if (animation.start != undefined && animation.start >= 0) newStart = animation.start;
            ...
            if (animation.reverse == undefined && details.reverse != undefined) animation.reverse = details.reverse;
            ...
            else if (animation.reverse && animation.start == 0) newStart = realduration;
        }
        ...
        try {
            /* code */
            object.playAnimation(realid); object.pauseAnimation();
        ...
        animationTimer = Animations.create({
            ...
            timing: (animation.timing != undefined) ? animation.timing: Animations.timing.linear,
            speed: (animation.speed != undefined) ? animation.speed: 1,
            onDraw: (perc) => { // 0 -1
                let newPerc = (reverse) ? 1 - perc : perc;
                let newTime = (reverse) ? newPerc * duration + newEnd : newPerc * duration + newStart;
    ...
    const getPos = ()=> { if (animationTimer) return animationTimer.getPos(); else return 0 }
    ...
    Object.defineProperties(o_animation, {
        animations: { get: () => { return animation_list; }, set: (v) => { } },
        track: { get: () => { return current_animation_id; }, set: (v) => { } },
    });
    ...
    const rgbs = ['albedo_ratio', 'emissive_ratio', 'diffuse_pbr_ratio',
        'diffuse_ibl_ratio', 'specular_pbr_ratio', 'specular_ibl_ratio',
        'ambient_ratio', 'diffuse_ratio', 'specular_ratio', 'sheen_color_ratio',
        'specular_glossiness_diffuse_ratio', 'specular_glossiness_specular_ratio'];

    const textures = ['ao_texture', 'specular_texture', 'metalness_texture',
        'roughness_texture', 'albedo_texture', 'emissive_texture',
        'normal_texture', 'opacity_texture', 'ambient_texture', 'diffuse_texture', 
        'clearcoat_texture', 'transmission_texture', 'sheen_color_texture', 'sheen_roughness_texture',
        'specular_glossiness_texture', 'specular_glossiness_diffuse_texture'];

    const videos = ['albedo_video', 'ambient_video'];

    const pbr_bundle_textures = ['ao_texture', 'roughness_texture', 'metalness_texture'];
    const transparency_bundle_textures = ['albedo_texture', 'diffuse_texture', 'opacity_texture'];
    ...
    const getTransformationValues = () => {
        const transformArray = ["position", "rotate", "scale", "anchor", "hud", "pivot", "autoscale", "groupMat", "controller", "hudscale"];
    ...
    const calculateTransformation = (obj) => {
        ...
        const pixelDensity = (transform.hud && Module.pixelDensity != undefined) ? Module.pixelDensity : 1;

        vec3.set(
            trv.scale, 
            transform.scale[0] * pixelDensity * transform.autoscale * localHudScale * globalHudScale,
            transform.scale[1] * pixelDensity * transform.autoscale * localHudScale * globalHudScale,
            transform.scale[2] * pixelDensity * transform.autoscale * localHudScale * globalHudScale
        )
        ...
        mat4.translate(trv.piv, trv.piv, vec3.fromValues(transform.pivot[0], transform.pivot[1], transform.pivot[2]));
        ...
        if (transform.hud) {
            ...
            obj.setParameter("hud_alignment", transform.anchor[0], transform.anchor[1], transform.anchor[2]);
        }
    ...
    const autoScale = ()=>{
        ...
        if (obj) {
            ...
            let largestScale = (extents.f1 > extents.f2) ? (extents.f1 > extents.f3) ? extents.f1 : extents.f3 : (extents.f2 > extents.f3) ? extents.f2 : extents.f3;
    ...
    const paintedProperty = (meshid, prop)=> {
        ...
        if (!obj) {
            ...
            if (type != undefined){
                ...
                let option = prop;
                switch (type) {
                    case "boolean": tValue = Boolean(obj.getParameterBool(an, option)); break;
                    case "string": tValue = (tValue != null && tValue != '') ? tValue : String(obj.getParameterString(an, option)); break;
                    case "float": tValue = Number(parseFloat(obj.getParameterFloat(an, option)).toFixed(3)); break;
                    case "vec3":
                        var arr = obj.getParameterVec3(an, option);
                        if (rgbs.includes(option)) {
                            tValue = [Number((arr.f1 * 255).toFixed(0)), Number((arr.f1 * 255).toFixed(0)), Number((arr.f1 * 255).toFixed(0))];
                        } else {
                            tValue = [Number(arr.f1.toFixed(3)), Number(arr.f2.toFixed(3)), Number(arr.f3.toFixed(3))];
                        }
    ...
    const setPropertyMesh = (meshid, prop, value, key) => {
        ...
        if (key == undefined) {
            ...
            addToUpdated(key, (isLoading) ? 'loaded' : 'changed', {meshid, prop, value})
        ...
        if (key == lastKey){
            ...
            if (lastKey != object.item.key) addToUpdated(lastKey, 'changed', {meshid, prop: prop + "_enabled",value : false})
    ...
    const render = (opts) => {
        ...
        let obj = scene.getObject(child.key);
        if (!obj) {
            const path = (!isNaN(child.id)) ? Module.ProjectManager.objPaths[String(child.id)] : String(child.id);
            try {
        ...
        for (var i in renderList) {
            ...
            switch (row.type) {
                ...
                case "mesh":
                    ...
                    if (_row.has(option)) {
                        ...
                        if (videos.includes(option)){
                            ...
                            if (video) {
                                const textureID = (video.textureId == null|| video.textureId == "") ? "" : video.textureId;
                                obj.setParameter(Number(meshid), option, textureID);
                            }

                        } else if (type == "object") {
                            if (rgbs.includes(option)) {
                                obj.setParameter(Number(meshid), option, value[0] / 255, value[1] / 255, value[2] / 255);
                            } else {
                                obj.setParameter(Number(meshid), option, value[0], value[1], value[2]);
                            }
                        } else {
                            if (textures.includes(option)) {
                                ...
                                if (pbr_bundle_textures.includes(option))
                                {
                                  ...
                                  pbrMeshRow.paths += (!scene.hasFSZip() ? Module.ProjectManager.path : "") + value + ";";
                                ...
                                else if (transparency_bundle_textures.includes(option))
                                {
                                  ...
                                  if (transparencyBundle.has(meshid)) transparencyMeshRow = transparencyBundle.get(meshid);
                                  else transparencyBundle.set(meshid, transparencyMeshRow);

                                  transparencyMeshRow.options += option + channel + ";";
                                  transparencyMeshRow.paths += (!scene.hasFSZip() ? Module.ProjectManager.path : "") + value + ";";
                                }
                                else obj.setParameter(Number(meshid), option + channel, (!scene.hasFSZip() ? Module.ProjectManager.path : "") + value);
                ...
                case "visible":
                    ...
                    break;
                case "show_shadow":
                    obj.setParameter("show_shadow", getLastValueInMap(getProperties(row.type)));
                    renderVisibility = true;

                    break;
                case "cast_shadow":
                    obj.setParameter("cast_shadow", getLastValueInMap(getProperties(row.type)));
                    renderVisibility = true;

                    break;

                case "front_facing":
                    obj.setParameter("front_facing", getLastValueInMap(getProperties(row.type)));
                    renderVisibility = true;
    ...
    Object.defineProperties(object, {
        ...
        show_shadow: { get: () => { return getProperty('show_shadow')[1]; }, set: (v) => { setProperty('show_shadow', v); } },
        cast_shadow: { get: () => { return getProperty('cast_shadow')[1]; }, set: (v) => { setProperty('cast_shadow', v); } },
        front_facing: { get: () => { return getProperty('front_facing')[1]; }, set: (v) => { setProperty('front_facing', v); } },
        ...
        frame: { get: () => { return getProperty('frame')[1]; }, set: (v) => { setProperty('frame', v); } },
        ...
        animation: { get: ()=> { return o_animation;} },

        animations: { get: ()=> { return customAnimations;} , set: (v) => { customAnimations = [...v]; getAnimationList(); } },
    ...
    Object.assign(object, {
        ...
        applyAutoScale: ()=> { autoscaleObject = true; },
        applyAutoPivot: ()=> { autospivotObject = true; },
