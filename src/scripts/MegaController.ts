export default `module.exports = (world) => {
  ...
  var inspectorData = {
    camera: {
      ...
      target: [
        0.018281878522202533, 0.47353737166849896, -0.023114974122790954,
      ],
      duration: 2000,
    },
    configurations: [],
  ...
  var onClick = function (node, object, meshid, button, x, y) {
    ...
    if (inspectorData.cameracomponent.length > 0) {
      try {
        cam = Module.ProjectManager.getObject(
          inspectorData.cameracomponent[0].file
        );
        // console.log(cam)
        finishing_camera = {
    ...
    if (inspectorData.duration > 0 && cam != undefined) {
      ...
      let d =
        6000 *
        vec3.distance(starting_camera.position, finishing_camera.position);
      if (d > dur) d = dur;
      else if (d < 100) d = 100;
    ...
    _inspector: () => {
      ...
      let op_camera = {
        ...
        value: {
          distance: 2.031473085436251,
          position: [
            -1.2651337339185942, 1.125515697724552, 1.4102905947753643,
          ],
          target: [
            0.018281878522202533, 0.47353737166849896, -0.023114974122790954,
          ],
      ...
      return [
        op_camera_component,
        op_duration,
        op_ConfigurationFolders,
        op_ObjectAnimations,
        op_ObjectAnimationSequence,
        op_Videos,
        op_pre,
        op_post,
      ];
